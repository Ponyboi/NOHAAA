...
public class XmlUtil{
	...
	public void WriteBaseBehaviour(BaseBehaviour baseBehaviour, string name){
		baseBehaviour.XmlWrite(this, name);
	}
	...
	public void WriteTransform(Transform transform, XmlElement parentXmlElement){
		WriteTransform(transform, parentXmlElement, "Transform");
	}

	public void WriteTransform(Transform transform, XmlElement parentXmlElement, string name){
		XmlElement xmlElement = GetNestedElement(parentXmlElement, name);
		if(transform != null){
			xmlElement.SetAttribute("ComponentExists", bool.TrueString);
			WriteVector3(GetNestedElement(xmlElement, "Position"), transform.localPosition);
			WriteVector3(GetNestedElement(xmlElement, "Rotation"), transform.localEulerAngles);//rotation.eulerAngles);
